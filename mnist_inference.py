...
def inference(input_tensor, train, regularizer):
    with tf.variable_scope('layer1-conv1'):
        ...
        # sigmoid1 = tf.nn.sigmoid(tf.nn.bias_add(conv1, conv1_biases))
        # tanh1 = tf.nn.tanh(tf.nn.bias_add(conv1, conv1_biases))
    ...
    with tf.variable_scope('layer3-conv2'):
        ...
        # sigmoid2 = tf.nn.sigmoid(tf.nn.bias_add(conv2, conv2_biases))
        # tanh2 = tf.nn.tanh(tf.nn.bias_add(conv1, conv1_biases))
    ...
    # 新增一层全连接层，将128个节点输出10个节点，即最终的分类结果
    with tf.variable_scope('layer7-fc3'):
        ...
        fc3_biases = tf.get_variable("bias",
                                     [OUTPUT_NODE],
                                     initializer=tf.constant_initializer(0.1))
