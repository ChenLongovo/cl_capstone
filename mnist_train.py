    ...
    with tf.Session(config=config) as sess:
        ...
        for i in range(TRAINING_STEPS):
            ...
            if i % 200 == 0:
                ...
                saver.save(sess, r"C:\Users\bksw\pycharmProjects\LeNet5\logs\model.ckpt", global_step=global_step)
                # 持久化一个简单的tensorflow模型。注意这里给出了global_step参数，这样可以让每个被
                # 保存模型的文件名末尾加上训练的轮数，比如“model.ckpt-1000” 表示训练1000轮之后得到的模型。
                # 通过 saver.save函数将tensorflow模型保存到了C:\Users\bksw\pycharmProjects\LeNet5\
