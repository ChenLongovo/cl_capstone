...
def evaluate(mnist):
    with tf.Graph().as_default() as g:  # 将默认图设为g
        ...
        # 计算误识率和拒识率
        num_total = tf.cast(tf.shape(correct_prediction)[0], tf.float32)

        num_errors = tf.count_nonzero(tf.not_equal(tf.argmax(y, 1), tf.argmax(y_, 1)), dtype=tf.float32) / num_total
        num_rejects = tf.count_nonzero(tf.logical_and(tf.equal(tf.argmax(y, 1), 0), tf.not_equal(tf.argmax(y_, 1), 0)),
                                       dtype=tf.float32) / num_total
        ...
        for i in range(2):  # 为了降低个人电脑的压力，此处只利用最后生成的模型对测试数据集做测试
            with tf.Session() as sess:
                ...
                ckpt = tf.train.get_checkpoint_state(r'C:\Users\bksw\pycharmProjects\LeNet5\logs')     # 文件夹即可
                if ckpt and ckpt.model_checkpoint_path:
                    ...
                    # 计算误识率和拒识率并输出
                    error_rate_value, reject_rate_value, accuracy_score = sess.run([num_errors, num_rejects, accuracy],
                                                                                   feed_dict=validate_feed)
                    print("After %s training step(s), 误识率 is %g and 拒识率 is %g and 正确率 = %g" % (
                            global_step, error_rate_value, reject_rate_value, accuracy_score))
